...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is HyperSparseFuseModule:
            # 确保 f 是一个列表，且长度正确
            if not isinstance(f, list) or len(f) != 2:
                raise ValueError("HyperFusionModule expects 'from' parameter to be a list of two indices.")
            # 从 ch 列表中获取两个输入通道数
            c1 = ch[f[0]]
            c2 = ch[f[1]]
            # 重新构建传递给 __init__ 的参数列表
            # 这里需要根据 HyperFusionModule 的 __init__ 定义来调整参数顺序
            hyper_args = [c1, c2]
            # 使用解包操作符 * 来传递参数
            m_ = m(*hyper_args)
        elif m is KFFusion:
            state_channels = ch[f[0]]
            observation_channels = ch[f[1]]
            args = [state_channels, observation_channels]
            c2 = state_channels + observation_channels
